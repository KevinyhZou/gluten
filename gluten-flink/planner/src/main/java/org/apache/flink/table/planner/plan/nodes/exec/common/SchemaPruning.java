...
public class SchemaPruning {
  ...
  public static RowType pruneSchema(RowType schema, List<RowField> requestFields) {
    List<LogicalType> rowTypes = requestFields
      .stream()
    ...
    List<RowField> schemaFields = schema.getFields()
      .stream()
  ...
  private static LogicalType sortLeftFieldsByRight(LogicalType left, LogicalType right) {
    if (left.equals(right)) {
      ...
      return new MapType(sortLeftFieldsByRight(leftKeyType, rightKeyType),
  ...
  private static LogicalType mergeInternal(LogicalType left, LogicalType right, BiFunction<RowType, RowType, RowType> f) {
    if (left instanceof ArrayType && right instanceof ArrayType) {
      ...
      String errMsg = String.format("Do not support merge %s and %s", left.getClass().getName(), 
